...
from indicators import StDev, MA, LastLow 
import pprint
...
def check_exit(position, day_count, day_limit):
    marker = '---'
    exit = False

    if day_count >= day_limit:
        ## allow for trade to stay open for no more than day_limit days
        exit = True
        marker = 'EXP'

    return exit, marker
        
## deep retrace strategy
## long only -
## close = lowest(close, previous N-day lows) and close > ma200
## enter on next day open -
## exit after holding position for Y days
## standard volatility stop, no entries allowed on end_of_week 
def bert_strategy(stock_name, wallet, retrace, horizon, start_from_dt=None, file_tag=None, use_raw=False):
    ...
    TRADE_TIME_LIMIT = horizon 
    stdev = StDev(sample_size=50)
    ma200 = MA(200)
    last_low = LastLow(RETRACE_LENGTH)
    ...
    for i in range(stock_df.shape[0]):
        ...
        sell_signal = False
        if ma200.count() > 1:
            #previous close < prior N day lows
            if prev_cls < last_low.valueAt(2) and prev_cls > ma200.valueAt(1):
                buy_signal = True

       

        trend_marker =  "UP" if ma200.count() > 1 and prev_cls > ma200.valueAt(1) else '---' 
        entry_marker = "---"
        exit_marker = "---"
        ...
        if position == 0:
            ...
            if position != 0:
                ...
                ## EXIT ON SAME BAR
                    ...
                    exit_triggered, exit_marker = check_exit(position, day_count, TRADE_TIME_LIMIT)
                    if exit_triggered:
                        trades.append(dict(InDate=idate,ExDate=idate,Position=position,Value=trade_mtm,Duration=1,Entry=entry,Exit=cls))
                        wallet += equity 
                        position_closed = True
...
if __name__ == '__main__':
    ...
    parser.add_argument("--retrace", help="retrace low count", type=int, default=6)
    parser.add_argument("--horizon", help="trading horizon (days)", type=int, default=10)
    ...
    bert_strategy(u.stock, u.wallet, u.retrace, u.horizon, u.start_from, u.tag, u.raw)
